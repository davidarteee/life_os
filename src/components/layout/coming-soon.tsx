...
import { moduleBullets } from "@/lib/i18n/content";
 ...
 * inside the real shell and design system. Bullet points are localized by module.
 */
export function ComingSoon({ titleKey, icon: Icon, points }: { titleKey: DictKey; icon: LucideIcon; points?: string[] }) {
  const { t, locale } = useT();
  const bullets = moduleBullets(locale, titleKey);
  const items = bullets.length > 0 ? bullets : points ?? [];
  return (
    <PageContainer>
